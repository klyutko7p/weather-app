<template>
    <div v-for="(city, index) in cities" :key="index">
        <div class="grid grid-cols-2 p-2 mx-auto mt-10 mb-10 bg-[#DDF5F9] max-w-sm cursor-pointer text-[#27495C] text-xl rounded-lg font-extrabold drop-shadow-2xl hover:scale-105 duration-300"
            @click="$router.push(`/weather/city=${city.name}&latitude=${getLat(city.lat)}&longitude=${getLng(city.lng)}`)">
            <img class="w-20" :src="linkToImg + city.country + '.svg'" alt="" />
            <div>
